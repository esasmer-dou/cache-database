...
[pscustomobject]@{
    hasMetricCardTarget = $content.Contains('data-target="diagnostics-section" data-nav-card="true"')
    hasDiagnosticsSection = $content.Contains('id="diagnostics-section"')
    hasDiagnosticsNav = $content.Contains('href="#diagnostics-section"')
    hasDiagnosticsTitle = $content.Contains('Tanılama Kayıtları')
