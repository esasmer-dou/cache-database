...
public final class CacheDatabaseAdminHttpServer implements AutoCloseable {
    ...
    private String renderMigrationPlanner(String language, String plannerPath) {
        ...
        return "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
                ...
                + "<div class=\"full planner-form-section\"><div class=\"planner-form-section-title\">" + escapeHtml(localized(normalizedLanguage, "1. Ekran tipi", "1. Screen type")) + "</div><div class=\"planner-form-section-copy\">"
                ...
                + "</div></div>"
                + "<details class=\"full planner-advanced\"><summary>" + escapeHtml(localized(normalizedLanguage, "İleri route ayarları", "Advanced route settings")) + "</summary><div class=\"planner-advanced-body\"><div class=\"planner-form-grid\">"
                ...
                + "</div></div></details>"
                + "<details class=\"full planner-advanced\"><summary>" + escapeHtml(localized(normalizedLanguage, "İleri scaffold ve comparison ayarları", "Advanced scaffold and comparison settings")) + "</summary><div class=\"planner-advanced-body\"><div class=\"planner-form-grid\">"
                ...
                + "function fillForm(data){Object.entries(data||{}).forEach(function(entry){const name=entry[0];const value=entry[1];const field=form.elements.namedItem(name);if(!field){return;}if(field instanceof RadioNodeList){return;}if(field.type==='checkbox'){field.checked=!!value;}else{field.value=value;}});syncDiscoverySelectors();syncPresetFromForm();}"
                ...
                + "function applyDiscoverySuggestion(index){const suggestion=discoverySuggestions[index];if(!suggestion||!suggestion.plannerRequest){return;}fillForm(suggestion.plannerRequest);syncPresetFromForm();setStatus('" + escapeJs(localized(normalizedLanguage, "Keşif önerisi forma uygulandı. İstersen değerleri kontrol edip planı üret.", "Discovery suggestion applied to the form. Check the values if you want, then generate the plan.")) + "');window.scrollTo({top:0,behavior:'smooth'});}"
                + "function applyDiscoveredObject(index, role){const item=discoveredTables[index];if(!item){return;}const target=role==='root'?'rootTableOrEntity':'childTableOrEntity';const field=plannerField(target);if(field){field.value=surfaceValue(item);syncDiscoverySelectors();setStatus('" + escapeJs(localized(normalizedLanguage, "Keşfedilen nesne forma işlendi. Şimdi diğer yüzeyi ve ekran tipini tamamlayabilirsin.", "The discovered object was applied to the form. You can now complete the other surface and the screen type.")) + "');}}"
                ...
                + "['rootTableOrEntity','childTableOrEntity'].forEach(function(name){const field=plannerField(name);if(field){field.addEventListener('change',function(){syncDiscoverySelectors();});}});"
                ...
                + "async function initializePlanner(){applyRoutePreset(routePresetField&&routePresetField.value||'timeline',true);await loadTemplate();await loadDemoDescriptor();await loadDiscovery(false);}"
