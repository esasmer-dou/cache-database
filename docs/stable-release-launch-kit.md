...
For `v0.3.1`, the official distribution channel is the GitHub Release asset:
...
CacheDB `v0.3.1` is a stable framework release focused on explicit active-route
...
- Release notes exist at `docs/releases/v0.3.1.md`.
...
- `Public Beta Readiness` and `Production Evidence` are green for the release
  commit.
- `Production GA Release Readiness` is green for `v0.3.1`.
- GitHub Release is not marked as prerelease.
- GitHub Release asset `cache-database-0.3.1-github-release.zip` is attached.
