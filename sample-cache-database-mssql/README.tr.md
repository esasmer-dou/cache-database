...
> Bu sürüm, genel kullanıma açık CacheDB Maven deposundaki değişmez `0.10.0`
...
flowchart LR
    ...
    Repo -->|"SourceRoute"| SQLServer[(SQL Server kalıcı geçmiş)]
    Repo -->|"Komut"| Stream["Redis Stream write-behind"]
    Stream --> Worker["Sınırlı kalıcılık işçisi"]
    Worker --> SQLServer
    SQLServer -->|"WarmRoute"| Warm["Ön yükleme / backfill işi"]
    Warm --> Redis
```
...
doctor plugin'ini değişmez `0.10.0` paketinden kimlik bilgisi istemeden çözümler:
...
Bu örnek SQL Server `tempdb` veritabanını kullanır ve kalıcı bir SQL Server
...
- Yedekleme, geri yükleme, Redis kaybı/yeniden hazırlama ve uygulamanın geri dönüş yollarını
  kanıtla.
- Production'da kalıcı bir uygulama veritabanı kullan; örnekteki `tempdb`
  kurulumunu kullanma.
