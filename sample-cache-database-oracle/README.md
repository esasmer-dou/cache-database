# CacheDB Oracle Sample

English | [Türkçe](README.tr.md)

[![Oracle provider evidence](https://github.com/esasmer-dou/cache-database/actions/workflows/production-evidence.yml/badge.svg?branch=main)](https://github.com/esasmer-dou/cache-database/actions/workflows/production-evidence.yml)
[![CacheDB 0.12.0](https://img.shields.io/badge/CacheDB-0.12.0-0b7285.svg)](https://github.com/esasmer-dou/cache-database/releases/tag/v0.12.0)

A production-oriented Spring Boot REST API that demonstrates CacheDB with Redis
8 and Oracle Database. The sample is intentionally explicit: operational routes use
a bounded Redis active data set, durable history stays in Oracle Database, and
growing lists use projections instead of full aggregates.

> This release consumes the immutable CacheDB `0.12.0` package from the public
...
route: seed durable Oracle Database rows, read the bounded archive route, warm the
matching Redis projection, read the active route, and inspect coverage and
durability signals. The sample does not hide an Oracle Database query behind a Redis
miss.
...
| Understand Redis vs Oracle Database behavior | [Runtime Contract](#runtime-contract) |
| See the declarative Java API | [Code Walkthrough](#code-walkthrough) |
| Warm existing Oracle Database rows | [Warm Existing Data](#warm-existing-data) |
...
| --- | --- |
| **BEST** | Define a bounded operational route, warm its entity or projection data, measure it, and keep archive/history reads on Oracle Database. |
| **ACCEPTABLE** | Use an explicit bounded Oracle Database route for infrequent data outside the Redis active set. |
...
flowchart LR
    ...
    Repo -->|"SourceRoute"| Oracle[(Oracle Database durable history)]
    Repo -->|"Command"| Stream["Redis Stream write-behind"]
    Stream --> Worker["Bounded persistence worker"]
    Worker --> Oracle
    Oracle -->|"WarmRoute"| Warm["Warm / backfill job"]
    Warm --> Redis
```
...
| Source route | A bounded repository method that reads Oracle Database explicitly |
| Warm/backfill | A controlled Oracle Database-to-Redis preparation job |
| Route coverage | Evidence that the required scope/window has been prepared in Redis |
| Write-behind | An accepted Redis command that is persisted to Oracle Database asynchronously |
...
processor. The Oracle starter already supplies the supported `ojdbc17` runtime
driver; do not add a second Oracle JDBC driver version to the same application.
...
processor, and doctor plugin anonymously from the immutable `0.12.0` package:
...
### 2. Start Redis and Oracle Database
...
| --- | --- | --- |
| Redis 8.2.1 | `127.0.0.1:56381` | Active entities, projections, indexes, streams, leases, and telemetry |
| Oracle Database Free 23 | `127.0.0.1:15212` | Durable source of truth |

Oracle can take longer to become healthy than Redis. Wait until
`docker compose ps` reports both services as healthy before starting the API.
...
Continue only when the status is `UP`. Readiness covers Redis, Oracle Database, and
...
# Bounded Oracle Database route
Invoke-RestMethod "http://127.0.0.1:8093/api/orders/archive?customerId=1&limit=10"
```

### 8. Open the tools

- Admin UI: `http://127.0.0.1:8093/cachedb-admin`
- Tuning snapshot: `http://127.0.0.1:8093/api/tuning`
- Scheduled warm status: `http://127.0.0.1:8093/api/warm/schedules`

Stop the local infrastructure without deleting Oracle Database data:

```powershell
docker compose down
```

Use `docker compose down -v` only when the local Oracle Database volume should be
...
| --- | --- | --- | --- |
| `save`, update, soft delete | Redis first, then Oracle Database write-behind | The command can enter Redis when admission policy permits | `202 Accepted` is not the same as SQL commit; inspect the receipt and readiness telemetry |
...
| Archive, export, audit history | Bounded Oracle Database source route | Reads Oracle Database directly | Apply row limits, deterministic ordering, indexes, and timeouts |
| Existing Oracle Database rows | Warm/backfill reads Oracle Database and hydrates Redis | No automatic startup import | Dry-run first, run a bounded warm job, then verify coverage |
| External Oracle Database write | Oracle Database changes first | Redis can become stale without a feed | Integrate outbox/CDC; periodic warm is reconciliation, not a replacement for event propagation |
...
Existing Oracle Database rows are not imported at startup. Use this sequence for an
existing system:
...
5. Verify route coverage and compare membership/order with Oracle Database.
6. Enable traffic gradually and retain the Oracle Database rollback path.
...
change-feed strategy. After cutover, every external Oracle Database writer must
...
| Report execution | `QUEUED/RUNNING/FAILED` or last 24 hours | Small job entity | Entity limit `5,000`, page `50` | Completed report history on Oracle Database |
| Security audit | Severe events from the last 24 hours | Small bounded entity list | Entity limit `2,000`, read admission disabled | Full audit archive on Oracle Database |
...
| Order reads | `GET /api/orders/{id}`, `GET /api/orders/high-value`, `GET /api/orders/archive` | Entity, ranked projection, Oracle Database source route |
| Products | `GET /api/products/active`, `GET /api/products/low-stock`, `PATCH /api/products/{id}/stock` | Projection reads and command |
| Shipments | `GET /api/shipments/active`, `GET /api/shipments/exceptions`, `GET /api/shipments/archive` | Projection and Oracle Database source routes |
...
numbers require realistic latency to Redis/Oracle Database, Kubernetes resource
limits, representative payloads, and expected concurrency.

## Oracle Database Production Notes

- Index every source-route predicate and deterministic sort suffix.
- Generate identifiers in the application or Redis command path. Do not rely on
  an Oracle sequence to assign an identifier after CacheDB has accepted a write.
- Map a numeric version column for optimistic write ordering. Stale versions are
  rejected by the version-guarded `MERGE` and delete statements.
- Oracle stores an empty string as `NULL`. The sample uses
  `cachedb.sql.oracle.empty-string-policy=REJECT`; normalize explicitly only
  when the domain contract treats empty and missing values as identical.
- Keep the sum of all Hikari pools below the Oracle service session budget, with
  reserved capacity for operations, migrations, and failover reconnects.
- Tune write-behind batch size against redo generation, row-lock duration, and
  delayed-network evidence. Larger batches are not automatically faster.
- Set bounded query timeouts for warm, archive, and migration queries. Keep
  route row limits below the provider's safe parameter and memory budgets.
- Provision outbox/checkpoint tables with migrations. Runtime DDL is disabled
  by default because Oracle DDL commits implicitly.
- Integrate outbox/CDC when another application can write the same tables.
- Use an Oracle service-name JDBC URL supplied by the platform team for RAC,
  Data Guard, or Application Continuity. Prove reconnect and retry behavior in
  the application's own staging topology.
- Before that staging test, run the framework's
  [physical Data Guard evidence lane](../docs/oracle-provider.md). It proves
  real redo apply and role transition through a multi-address Oracle JDBC
  service-name descriptor, then reinstates the former primary and requires final
  no-gap/zero-lag readiness. It does not replace RAC/SCAN/FAN/FCF,
  application-specific network evidence, or an explicit production RPO decision.
- Prove backup, restore, Redis loss/rebuild, and application rollback paths.
- Treat Oracle Database as the durable source of truth even when Redis serves the
  operational route.
...
- [ ] Redis uses explicit `maxmemory`, `noeviction`, alerting, and capacity
  headroom.
- [ ] Oracle Database session and HikariCP budgets are calculated per pod and
  for the full replica count.
...
| Readiness is `DOWN` | Redis, Oracle Database, dead-letter, recovery, or backlog condition failed | Inspect readiness details and logs; do not route traffic yet |
