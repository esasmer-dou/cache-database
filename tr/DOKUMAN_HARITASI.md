...
| Mevcut PostgreSQL + ORM uygulamasını nasıl taşırım? | [Geçiş Planlayıcı](docs/migration-planner.md) |
| PostgreSQL CacheDB dışında değişirse Redis'i nasıl güncel tutarım? | [Outbox ve CDC Apply Runner](docs/outbox-cdc-apply-runner.md) |
| CacheDB MSSQL veya başka SQL veritabanını destekleyebilir mi? | [Veritabanı Provider SPI Yönü](docs/veritabani-provider-spi.md) |
| Production'a çıkmadan önce hangi kanıtlar gerekir? | [Production Test Raporu](docs/production-test-report.md) |
| Açık beta mı, GA mı? | [Açık Beta Hazırlık Durumu](docs/public-beta-readiness.md) ve [Production GA Criteria](../PRODUCTION_GA_CRITERIA.md) |
...
Hedef: Redis, PostgreSQL, worker, leader lease, failover, CI evidence ve
operasyonel gözlemlenebilirlik sınırlarını anlamak.
...
| [Geçiş Planlayıcı](docs/migration-planner.md) | Mevcut PostgreSQL sistemlerinden geçiş akışı |
...
| [Veritabanı Provider SPI Yönü](docs/veritabani-provider-spi.md) | PostgreSQL, MSSQL ve ilerideki SQL dialect'leri için storage provider sınırı |
...
| [Public Beta Launch Kit](docs/public-beta-launch-kit.md) | GitHub açıklaması, release mesajı ve public beta konumlandırması |
