...
| --- | --- | --- | --- |
| Mimari | Redis-first, SQL-provider-backed model net ve tutarlı | Klasik ORM'e göre operasyonel modeli daha karmaşık | Takımlar sistemin gerçekten sunduğundan daha güçlü consistency varsayabilir |
...
| Redis boundedness | Guardrail, hard-limit, shedding, runtime profile switching ve degraded fallback var | Uzun soak testlerinde bounded davranış henüz kanıtlı değil | Kaynak veritabanı drain geri kalırsa uzun overload altında Redis yine baskı görebilir |
...
| --- | --- | --- |
| 1 | Hedef donanımda sustained drain benchmark | Mevcut ana darboğaz hâlen kaynak veritabanı drain capacity |
...
| Aşama C | Sustained throughput'u kanıtlamak | hedef donanımda drain benchmark, kaynak veritabanı tuning raporu, backlog slope analizi |
...
- kaynak veritabanı yavaşlaması veya geçici outage ile replay recovery
