...
- Redis düşük gecikmeli okuma/yazma hızlandırma katmanıdır.
...
| Redis memory davranışı | Bellek ve Redis Penceresi Kuralları |
...
- `ProductEntity` Redis'te tutulabilir.
...
- Güncel shipment status Redis'te tutulabilir.
...
### Query 1: Redis'teki Müşteriyi Id İle Okuma
...
### Query 2: 1.000 Satırlık Redis Projection Penceresinden Son 10 Siparişi Okuma
...
- Redis 1.000 satırlık summary penceresi tutabilir.
- Sorgu yalnızca istenen 10 summary kaydını döndürür.
- Bu kritik liste route'u için kaynak veritabanı gerekmez.
...
- Redis pencereleri soğuk arşiv okumalarıyla kirlenmez.
...
### Update 1: Redis'teki Entity'yi Full State İle Değiştirme
...
- Mevcut customer Redis'te bulunduğu için Redis'ten okunur.
...
Kaynak veritabanında batch SQL kullan, ardından etkilenen kritik route'ları açıkça yenile
veya invalidate et.
...
### Delete 1: Redis'teki Siparişi Silme
...
- CacheDB yalnızca sınırlı Redis penceresini ve okuma modellerini tutarlı tutmalıdır.
...
- Müşteri başına son 1.000 summary Redis'te tutulur.
...
- Yalnızca aktif raporlama penceresini Redis'te tut.
...
## Bellek ve Redis Penceresi Kuralları

CacheDB bütün veritabanını Redis'te tutmak zorunda değildir.

BEST:

- Sık okunan root entity'ler Redis'te kalır.
...
`COUNT_WINDOW`, Redis'teki veri kümesi son erişilen veya son yazılan kayıtlardan oluşuyorsa kullanılır.
...
`TIME_WINDOW`, Redis'e alınacak veri iş zamanına göre belirleniyorsa kullanılır. Örnek: "son 90 günün order kayıtları". Bu kural için `entityTtlSeconds` kullanma; TTL, Redis'e yazılma anından itibaren işler.
...
`STATE_WINDOW`, yalnız belirli iş durumlarının Redis'te kalması gerektiğinde kullanılır.
...
`COMPOSITE`, gerçek route birden fazla Redis kabul kuralına bağlıysa kullanılır.
...
kalmalıdır. VIP müşteri kayıtları da Redis'te kalacaksa VIP kuralını küçük ve
...
- Page size Redis penceresinin altında kalır.
...
- Veri Redis'te mi, eski geçmiş mi?
